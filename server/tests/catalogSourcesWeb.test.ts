...
const TWO_HOSTS = JSON.stringify([{ title: 'Agent Reliability Engineer', oneLineSummary: 'Keeps AI agents dependable in production.', evidenceUrls: ['https://jobs.alpha.test/1', 'https://www.beta.test/2'] }]);
...
describe('web research for emerging titles', () => {
  ...
  it('asks the Responses API with web search, the configured model and the key', async () => {
    const { http, calls } = fakeHttp(() => new Response(responsesBody(TWO_HOSTS)));
  ...
  it('keeps a title with evidence from two distinct hosts', async () => {
    const { http } = fakeHttp(() => new Response(responsesBody(TWO_HOSTS)));
  ...
  it('drops a title whose evidence is all from one host', async () => {
    const oneHost = JSON.stringify([{ title: 'Vibe Coder', oneLineSummary: 'x', evidenceUrls: ['https://alpha.test/1', 'https://www.alpha.test/2'] }]);
    const { http } = fakeHttp(() => new Response(responsesBody(oneHost)));
    expect((await researchEmergingTitles(http, options())).candidates).toEqual([]);
  });

  it('counts url_citation annotations inside a title\'s entry as its evidence', async () => {
    const text = JSON.stringify([
      { title: 'Prompt Librarian', oneLineSummary: 'Curates prompts.', evidenceUrls: ['https://alpha.test/1'] },
      { title: 'Eval Engineer', oneLineSummary: 'Builds evals.', evidenceUrls: ['https://alpha.test/9'] },
    ]);
    const first = text.indexOf('Prompt Librarian');
    const { http } = fakeHttp(() => new Response(responsesBody(text, [{ type: 'url_citation', url: 'https://gamma.test/post', start_index: first, end_index: first + 5 }])));
    const result = await researchEmergingTitles(http, options());
    expect(result.candidates.map((c) => c.title)).toEqual(['Prompt Librarian']);
  });

  it('keeps every evidence link for the reviewer', async () => {
    const { http } = fakeHttp(() => new Response(responsesBody(TWO_HOSTS)));
  ...
  it('reads a reply wrapped in a json code fence', async () => {
    const fenced = ['```json', TWO_HOSTS, '```'].join('\n');
    const { http } = fakeHttp(() => new Response(responsesBody(fenced)));
  ...
  it('drops entries that fail validation and keeps the rest', async () => {
    const mixed = JSON.stringify([{ oneLineSummary: 'no title' }, { title: 'Agent Reliability Engineer', oneLineSummary: 'ok', evidenceUrls: ['https://a.test/1', 'https://b.test/1'] }]);
    const { http } = fakeHttp(() => new Response(responsesBody(mixed)));
    expect((await researchEmergingTitles(http, options())).candidates.map((c) => c.title)).toEqual(['Agent Reliability Engineer']);
  });

  it('ignores evidence that is not a web link', async () => {
    const odd = JSON.stringify([{ title: 'Odd Title', oneLineSummary: 'x', evidenceUrls: ['javascript:alert(1)', 'https://a.test/1'] }]);
    const { http } = fakeHttp(() => new Response(responsesBody(odd)));
    expect((await researchEmergingTitles(http, options())).candidates).toEqual([]);
  });
