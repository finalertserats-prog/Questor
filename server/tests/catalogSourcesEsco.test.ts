...
function searchBody(uris: readonly [string, string][], total = 2942) {
  return JSON.stringify({ total, _embedded: { results: uris.map(([uri, title]) => ({ uri, title, preferredLabel: { en: title, de: 'x' } })) } });
}
...
describe('paging ESCO occupations', () => {
  it('asks the search endpoint for one page of English occupations', async () => {
    const { http, requested } = fakeHttp(standardRoutes);
    await fetchEscoPage(http, { baseUrl: BASE, offset: 50, limit: 25, delayMs: 10 });
    expect(Object.fromEntries(requested[0].searchParams)).toEqual({ type: 'occupation', language: 'en', text: '', limit: '25', offset: '50' });
  });
  ...
  it('reads English alternative labels and description from each occupation resource', async () => {
    ...
    const page = await fetchEscoPage(http, { baseUrl: BASE, offset: 0, limit: 25, delayMs: 10 });
  ...
  it('reports the total so the cursor can wrap', async () => {
    const { http } = fakeHttp(standardRoutes);
    expect((await fetchEscoPage(http, { baseUrl: BASE, offset: 0, limit: 25, delayMs: 10 })).total).toBe(2942);
  });

  it('pauses between requests, one request at a time', async () => {
    const { http, sleeps } = fakeHttp(standardRoutes);
    await fetchEscoPage(http, { baseUrl: BASE, offset: 0, limit: 25, delayMs: 10 });
  ...
  it('fails the page when the search itself fails', async () => {
    ...
    await expect(fetchEscoPage(http, { baseUrl: BASE, offset: 0, limit: 25, delayMs: 10 })).rejects.toThrow(/400/);
  });

  it('treats an unexpected search body as an empty page', async () => {
    const { http } = fakeHttp(() => new Response(JSON.stringify({ surprise: true })));
    expect(await fetchEscoPage(http, { baseUrl: BASE, offset: 0, limit: 25, delayMs: 10 })).toEqual({ candidates: [], total: 0, errors: [] });
  });
