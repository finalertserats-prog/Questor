...
rolesRouter.get('/:id', asyncHandler(async (req, res) => {
  const role = await getRole(req.auth!.tenantId, req.params.id);
...
rolesRouter.put('/:id/scorecard', asyncHandler(async (req, res) => {
  const role = await getRole(req.auth!.tenantId, req.params.id);
...
rolesRouter.post('/:id/approve', asyncHandler(async (req, res) => {
  const role = await getRole(req.auth!.tenantId, req.params.id);
  const latest = await prisma.roleScorecardVersion.findFirst({ where: { roleId: role.id }, orderBy: { version: 'desc' } });
...
async function getRole(tenantId: string, id: string) {
