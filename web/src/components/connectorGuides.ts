...
export const CONNECTOR_GUIDES: readonly ConnectorGuide[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Add the three variables to server/.env and restart the server.',
    ],
    envVars: [
    ...
    permissions: [
      ...
      'user:read:user:admin (look up the host user)',
  ...
  },
  {
    ...
    steps: [
      ...
      'To create meetings on behalf of users, a Teams admin must also create an application access policy for this app (Teams PowerShell: New-CsApplicationAccessPolicy).',
      'Add the three variables to server/.env and restart the server.',
    ],
    envVars: [
    ...
    permissions: [
      ...
      'Calls.JoinGroupCall.All (Application) — let a bot join a meeting',
      'Calls.AccessMedia.All (Application) — real-time media for the bot',
      'Admin consent is required for all of these.',
    ],
    callbacks: [],
    callbackNote: APP_ONLY_AUTH,
  ...
  },
  {
    ...
    steps: [
      'In the Google Cloud console create or pick a project and enable the "Google Meet REST API".',
      ...
      'Pick the Workspace user that meetings will be created as (for example an HR mailbox).',
    ...
    docsLabel: 'Google Meet REST API overview',
  },
  {
    id: 'email-sendgrid',
    group: 'email',
  ...
  },
  {
    id: 'ats',
    group: 'ats',
    title: 'ATS (server-configured, one organisation)',
    vendorAccount: 'An ATS that exposes a REST API and issues API keys (Greenhouse Harvest is the named example).',
    steps: [
      'Most organisations connect their own ATS in Settings; use these variables only to bind one ATS to one organisation from the server.',
      'Create an API key in your ATS with read access to jobs/requisitions and candidates, and write access to candidate assessments.',
      'Set ATS_BASE_URL to the API root. The connector calls <base>/requisitions/<id>, <base>/candidates/<id> and <base>/candidates/<id>/assessments with a Bearer token.',
      'Set ATS_TENANT_ID to the organisation the ATS belongs to. Without it the variables are ignored.',
    ...
    envVars: [
      ...
      { name: 'ATS_TENANT_ID', placeholder: '<organisation id>', note: 'required; the only organisation that may use this ATS' },
    ],
    permissions: ['Read requisitions/jobs', 'Read candidates', 'Write candidate assessments'],
    callbacks: [],
    callbackNote: 'None needed.',
