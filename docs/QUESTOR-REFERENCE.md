...
| `ANONYMISE_SWEEP_ENABLED` | off | The daily **anonymisation** sweep: the other answer to storage limitation. Keeps the interview and irreversibly removes the identifying details Questor holds about the candidate — name, address, phone, LinkedIn — from every row they appear in, including the transcript. Files, the CV, the ATS link, certificates and invitations are deleted outright; the candidate's own audit rows keep their actor, action, entity and timestamp but lose their before/after payloads. Preview first at `GET /api/admin/anonymisation/preview` | Off — opt-in, because it cannot be undone |
